...
for t = 1:20
    for j = 1:20
        ...
        for i = 1:5
            ...
            if(t==j)
                modelSVM = fitcsvm(trainData,trainLabels,'KernelFunction','linear','Standardize', true);
            else 
                modelSVM = fitcsvm(trainData,trainLabels,'KernelFunction','linear','OptimizeHyperparameters','auto','Standardize', true);
            end
            close all
        %% Testing Stage
        ...
        % For every image we will infer a label from the model
            for l = 1:size(testData,1)
                ...
                classificationResult(l,1) = predict(modelSVM, testNumber);
            end
